...
#    idx_features_labels['TotalMonthsOverdue'] = (idx_features_labels['TotalMonthsOverdue']-idx_features_labels['TotalMonthsOverdue'].mean())/idx_features_labels['TotalMonthsOverdue'].std()
    ...
    idx_train = np.append(label_idx_0[:int(0.6 * len(label_idx_0)),], label_idx_1[:int(0.5 * len(label_idx_1))])
...
#
    ...
    idx_train = np.append(label_idx_0[:min(int(0.6 * len(label_idx_0)), label_number//2)], label_idx_1[:min(int(0.6 * len(label_idx_1)), label_number//2)])
    idx_val = np.append(label_idx_0[int(0.6 * len(label_idx_0)):int(0.8 * len(label_idx_0))], label_idx_1[int(0.6 * len(label_idx_1)):int(0.8 * len(label_idx_1))])
    idx_test = np.append(label_idx_0[int(0.8 * len(label_idx_0)):], label_idx_1[int(0.8 * len(label_idx_1)):])
...
def load_facebook(dataset, sens_attr, predict_attr, path="../dataset/facebook/", label_number=1000, sens_number=500):
    """Load data"""
    print('Loading {} dataset from {}'.format(dataset, path))
...
def load_pokec(dataset, sens_attr, predict_attr, path="dataset/pokec/", label_number=1000, sens_number=500, seed=19,
    ...
    idx_features_labels = pd.read_csv(os.path.join(path, "region_job_2.csv".format(dataset)))
    header = list(idx_features_labels.columns)
    header.remove("user_id")
    ...
    idx = np.array(idx_features_labels["user_id"], dtype=int)
    idx_map = {j: i for i, j in enumerate(idx)}
    edges_unordered = np.genfromtxt(os.path.join(path, "region_job_2_relationship.txt".format(dataset)), dtype=int)
...
def load_data(path='/Users/franciscosantos/Documents/FACS-GCN/data/', dataset='tagged', seed=80):
